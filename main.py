...
def auth_user(db, username: str, pw: str):
...
@app.post("/register")
async def register_user(username: str, pw: str):
    user = get_user(db, username)
    if user:
        raise HTTPException(400, detail="Username already taken")
    hashed_pw = get_pw_hash(pw)
    created_user = {"username": username, "hashed_password": hashed_pw}
    db[username] = created_user
